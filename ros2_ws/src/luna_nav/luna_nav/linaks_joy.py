#!/usr/bin/env python3
"""
Joystick → LINAK actuator bridge.

Button layout (Xbox controller, all remappable via parameters):
  Y (3)  — run full excavation sequence
  B (1)  — run full dump sequence
  X (2)  — stop all actuators immediately
  LT (axis 2) — held: manual shoulder extend (run out)
  LB (4)      — held: manual shoulder retract (run in)
  RT (axis 5) — held: manual wrist extend (run out)
  RB (5)      — held: manual wrist retract (run in)

Position targets (mm) and tolerance are set via ROS parameters.
"""
