...
Parameters:
  ...
  min_frontier_size   -- minimum cells for a valid frontier cluster (default 5)
  rate_hz             -- detection loop rate (default 1.0)
...
class FrontierExplorerNode(Node):
    ...
    def __init__(self):
        ...
        self._min_frontier = int(self.get_parameter('min_frontier_size').value or 5)
        rate = float(self.get_parameter('rate_hz').value or 1.0)
        ...
        if self._state == ExplorationState.NAVIGATING:
            if self._current_goal is not None:
                ...
                if math.hypot(dx, dy) < self._goal_tol:
                    self.get_logger().info('Reached frontier goal, re-scanning.')
                    self._state = ExplorationState.SCANNING
    ...
    def _detect_frontiers(self, robot_xy):
        ...
        # BFS over free space from robot cell
        queue = deque()
        queue.append((rc, cc))
        visited[rc, cc] = True
        ...
        # Cluster frontier cells via connected-components BFS
    ...
    def _select_frontier(self, frontiers, robot_xy):
        """Pick the nearest non-blacklisted frontier centroid."""
        best = None
        best_dist = float('inf')
        for fx, fy, _size in frontiers:
    ...
    def _cancel_nav_goal(self):
        if self._nav_client.server_is_ready():
            self._nav_client._cancel_goal_async(None)
    ...
    def _publish_frontier_markers(self, frontiers, robot_xy):
        ...
        for i, (fx, fy, size) in enumerate(frontiers):
            ...
            m.color.b = 0.8
            m.color.a = 0.7
            m.lifetime = DurationMsg(sec=3, nanosec=0)
            ma.markers.append(m)
        ...
        if self._current_goal:
            ...
            g.color.b = 0.0
            g.color.a = 0.9
            g.lifetime = DurationMsg(sec=3, nanosec=0)
            ma.markers.append(g)
