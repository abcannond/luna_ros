#pragma message("#include \"luna_controller_parameters.hpp\" is deprecated. Use #include <luna_control/luna_controller_parameters.hpp> instead.")
...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        std::vector<std::string> left_back_wheel_names = {};
        std::vector<std::string> left_front_wheel_names = {};
        std::vector<std::string> right_back_wheel_names = {};
        std::vector<std::string> right_front_wheel_names = {};
        std::vector<std::string> left_back_pod_names = {};
        std::vector<std::string> left_front_pod_names = {};
        std::vector<std::string> right_back_pod_names = {};
        std::vector<std::string> right_front_pod_names = {};
        double wheel_track = 0.0;
        double wheel_base = 0.0;
        double wheel_radius = 0.0;
        double allowed_steer_pod_driving_angle = 3.1416;
  ...
  public:
    ...
    : ParamListener(parameters_interface, rclcpp::get_logger("luna_controller"), prefix) {
    ...
    StackParams get_stack_params() {
      ...
      output.allowed_steer_pod_driving_angle = params.allowed_steer_pod_driving_angle;
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        if (param.get_name() == (prefix_ + "left_back_wheel_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.left_back_wheel_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "left_front_wheel_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.left_front_wheel_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "right_back_wheel_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.right_back_wheel_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "right_front_wheel_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.right_front_wheel_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "left_back_pod_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.left_back_pod_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "left_front_pod_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.left_front_pod_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "right_back_pod_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.right_back_pod_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "right_front_pod_names")) {
            if(auto validation_result = not_empty<std::string>(param);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.right_front_pod_names = param.as_string_array();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        ...
        if (param.get_name() == (prefix_ + "allowed_steer_pod_driving_angle")) {
            if(auto validation_result = gt<double>(param, 0.0);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.allowed_steer_pod_driving_angle = param.as_double();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "left_back_wheel_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the left back pod wheels' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.left_back_wheel_names);
          parameters_interface_->declare_parameter(prefix_ + "left_back_wheel_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "left_front_wheel_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the left front pod wheels' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.left_front_wheel_names);
          parameters_interface_->declare_parameter(prefix_ + "left_front_wheel_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "right_back_wheel_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the right back pod wheels' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.right_back_wheel_names);
          parameters_interface_->declare_parameter(prefix_ + "right_back_wheel_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "right_front_wheel_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the right front pod wheels' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.right_front_wheel_names);
          parameters_interface_->declare_parameter(prefix_ + "right_front_wheel_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "left_back_pod_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the left back pod steering pods' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.left_back_pod_names);
          parameters_interface_->declare_parameter(prefix_ + "left_back_pod_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "left_front_pod_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the left front pod steering pods' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.left_front_pod_names);
          parameters_interface_->declare_parameter(prefix_ + "left_front_pod_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "right_back_pod_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the right back pod steering pods' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.right_back_pod_names);
          parameters_interface_->declare_parameter(prefix_ + "right_back_pod_names", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "right_front_pod_names")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the right front pod steering pods' joints";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.right_front_pod_names);
          parameters_interface_->declare_parameter(prefix_ + "right_front_pod_names", parameter, descriptor);
      }
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "wheel_base")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Shortest distance between the front and back wheels. If this parameter is wrong, the robot will not behave correctly in curves.";
          descriptor.read_only = false;
          descriptor.floating_point_range.resize(1);
          descriptor.floating_point_range.at(0).from_value = 0.0;
          descriptor.floating_point_range.at(0).to_value = std::numeric_limits<double>::max();
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "allowed_steer_pod_driving_angle")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "When all of the steering pods re this close to the target steering angle, the drive wheels are allowed to spin. If any steering pod is outside of +- this angle of their target, the robot will remain stationary. Angle measured in radians.";
          descriptor.read_only = false;
          descriptor.floating_point_range.resize(1);
          descriptor.floating_point_range.at(0).from_value = 0.0;
          descriptor.floating_point_range.at(0).to_value = std::numeric_limits<double>::max();
          auto parameter = to_parameter_value(updated_params.allowed_steer_pod_driving_angle);
          parameters_interface_->declare_parameter(prefix_ + "allowed_steer_pod_driving_angle", parameter, descriptor);
      }
      ...
      rclcpp::Parameter param;
      param = parameters_interface_->get_parameter(prefix_ + "left_back_wheel_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'left_back_wheel_names': {}", validation_result.error()));
      }
      updated_params.left_back_wheel_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "left_front_wheel_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'left_front_wheel_names': {}", validation_result.error()));
      }
      updated_params.left_front_wheel_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "right_back_wheel_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'right_back_wheel_names': {}", validation_result.error()));
      }
      updated_params.right_back_wheel_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "right_front_wheel_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'right_front_wheel_names': {}", validation_result.error()));
      }
      updated_params.right_front_wheel_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "left_back_pod_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'left_back_pod_names': {}", validation_result.error()));
      }
      updated_params.left_back_pod_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "left_front_pod_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'left_front_pod_names': {}", validation_result.error()));
      }
      updated_params.left_front_pod_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "right_back_pod_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'right_back_pod_names': {}", validation_result.error()));
      }
      updated_params.right_back_pod_names = param.as_string_array();
      param = parameters_interface_->get_parameter(prefix_ + "right_front_pod_names");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'right_front_pod_names': {}", validation_result.error()));
      }
      updated_params.right_front_pod_names = param.as_string_array();
      ...
      updated_params.wheel_radius = param.as_double();
      param = parameters_interface_->get_parameter(prefix_ + "allowed_steer_pod_driving_angle");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = gt<double>(param, 0.0);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'allowed_steer_pod_driving_angle': {}", validation_result.error()));
      }
      updated_params.allowed_steer_pod_driving_angle = param.as_double();
