...
## 7. Optional: frontier detection (WFD)
...
If the mission is **purely zone-and-waypoint** driven, frontier nodes can remain **disabled** in competition.

---

## 8. Implementation checklist (later)

Use this as a task list when implementation starts:

- [ ] Freeze **frame conventions** (`map`, `odom`, `base_link`, optional `arena`).
- [ ] Lock **ArUco** dictionary, IDs, and sizes to field / rules.
- [ ] Complete **camera intrinsics + camera→base_link** extrinsics on hardware.
- [ ] Implement **zones.yaml** and **zone_classifier** node (TF + point-in-polygon + hysteresis).
- [ ] Wire **fiducial pose** → initial global pose or EKF / AMCL (choose fusion strategy).
- [ ] Implement **mission FSM/BT** + Nav2 goals per phase.
- [ ] Add **logging** (rosbag) for `/map`, `/tf`, goals, and zone transitions for debug.
- [ ] (Optional) Frontier node for mapping sessions only.
